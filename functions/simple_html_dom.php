...
define('IDNICH_HDOM_TYPE_ELEMENT'	, 1);
define('IDNICH_HDOM_TYPE_COMMENT'	, 2);
define('IDNICH_HDOM_TYPE_TEXT'		, 3);
define('IDNICH_HDOM_TYPE_ENDTAG'	, 4);
define('IDNICH_HDOM_TYPE_ROOT'		, 5);
define('IDNICH_HDOM_TYPE_UNKNOWN'	, 6);
define('IDNICH_HDOM_QUOTE_DOUBLE'	, 0);
define('IDNICH_HDOM_QUOTE_SINGLE'	, 1);
define('IDNICH_HDOM_QUOTE_NO'		, 3);
define('IDNICH_HDOM_INFO_BEGIN'		, 0);
define('IDNICH_HDOM_INFO_END'		, 1);
define('IDNICH_HDOM_INFO_QUOTE'		, 2);
define('IDNICH_HDOM_INFO_SPACE'		, 3);
define('IDNICH_HDOM_INFO_TEXT'		, 4);
define('IDNICH_HDOM_INFO_INNER'		, 5);
define('IDNICH_HDOM_INFO_OUTER'		, 6);
define('IDNICH_HDOM_INFO_ENDSPACE'	, 7);

defined('IDNICH_DEFAULT_TARGET_CHARSET') || define('IDNICH_DEFAULT_TARGET_CHARSET', 'UTF-8');
defined('IDNICH_DEFAULT_BR_TEXT') || define('IDNICH_DEFAULT_BR_TEXT', "\r\n");
defined('IDNICH_DEFAULT_SPAN_TEXT') || define('IDNICH_DEFAULT_SPAN_TEXT', ' ');
defined('IDNICH_MAX_FILE_SIZE') || define('IDNICH_MAX_FILE_SIZE', 600000);
define('IDNICH_HDOM_SMARTY_AS_TEXT', 1);

function idnich_file_get_html(
	$url,
	...
	$target_charset = IDNICH_DEFAULT_TARGET_CHARSET,
	$stripRN = true,
	$defaultBRText = IDNICH_DEFAULT_BR_TEXT,
	$defaultSpanText = IDNICH_DEFAULT_SPAN_TEXT)
{
	if($maxLen <= 0) { $maxLen = IDNICH_MAX_FILE_SIZE; }

	$dom = new idnich_simple_html_dom(
		null,
		$lowercase,
		$forceTagsClosed,
		$target_charset,
	...
	$defaultSpanText = IDNICH_DEFAULT_SPAN_TEXT)
{
	...
	if (empty($str) || strlen($str) > IDNICH_MAX_FILE_SIZE) {
		$dom->clear();
		return false;
	}
...
function idnich_dump_html_tree($node, $show_attr = true, $deep = 0)
{
	$node->dump($node);
}

class idnich_simple_html_dom_node
{
	public $nodetype = IDNICH_HDOM_TYPE_TEXT;
	public $tag = 'text';
	public $attr = array();
	...
	function dump_node($echo = true)
	{
		...
		$string .= ' IDNICH_HDOM_INNER_INFO: ';

		if (isset($node->_[IDNICH_HDOM_INFO_INNER])) {
			$string .= "'" . $node->_[IDNICH_HDOM_INFO_INNER] . "'";
		} else {
			$string .= ' NULL ';
		}
	...
	function innertext()
	{
		if (isset($this->_[IDNICH_HDOM_INFO_INNER])) {
			return $this->_[IDNICH_HDOM_INFO_INNER];
		}

		if (isset($this->_[IDNICH_HDOM_INFO_TEXT])) {
			return $this->dom->restore_noise($this->_[IDNICH_HDOM_INFO_TEXT]);
		}
	...
	function outertext()
	{
		...
		if (isset($this->_[IDNICH_HDOM_INFO_OUTER])) {
			return $this->_[IDNICH_HDOM_INFO_OUTER];
		}
		...
		if ($this->dom && $this->dom->nodes[$this->_[IDNICH_HDOM_INFO_BEGIN]]) {
			$ret = $this->dom->nodes[$this->_[IDNICH_HDOM_INFO_BEGIN]]->makeup();
		}

		if (isset($this->_[IDNICH_HDOM_INFO_INNER])) {
			// todo: <br> should either never have IDNICH_HDOM_INFO_INNER or always
			if ($this->tag !== 'br') {
				$ret .= $this->_[IDNICH_HDOM_INFO_INNER];
			}
		} elseif ($this->nodes) {
			foreach ($this->nodes as $n) {
		...
		if (isset($this->_[IDNICH_HDOM_INFO_END]) && $this->_[IDNICH_HDOM_INFO_END] != 0) {
			$ret .= '</' . $this->tag . '>';
		}
	...
	function text()
	{
		...
		switch ($this->nodetype) {
			case IDNICH_HDOM_TYPE_TEXT: return $this->dom->restore_noise($this->_[IDNICH_HDOM_INFO_TEXT]);
			case IDNICH_HDOM_TYPE_COMMENT: return '';
			case IDNICH_HDOM_TYPE_UNKNOWN: return '';
		}
		...
		// In rare cases, (always node type 1 or IDNICH_HDOM_TYPE_ELEMENT - observed
	...
	function makeup()
	{
		...
		foreach ($this->attr as $key => $val) {
			...
			$ret .= $this->_[IDNICH_HDOM_INFO_SPACE][$i][0];

			//no value attr: nowrap, checked selected...
			if ($val === true) {
				$ret .= $key;
			} else {
				switch ($this->_[IDNICH_HDOM_INFO_QUOTE][$i])
				{
					case IDNICH_HDOM_QUOTE_DOUBLE: $quote = '"'; break;
					case IDNICH_HDOM_QUOTE_SINGLE: $quote = '\''; break;
					default: $quote = '';
				}

				$ret .= $key
				. $this->_[IDNICH_HDOM_INFO_SPACE][$i][1]
				. '='
				. $this->_[IDNICH_HDOM_INFO_SPACE][$i][2]
				. $quote
				. $val
				. $quote;
			}
		...
		return $ret . $this->_[IDNICH_HDOM_INFO_ENDSPACE] . '>';
	}
	...
	function find($selector, $idx = null, $lowercase = false)
	{
		...
		for ($c = 0; $c < $count; ++$c) {
			...
			if (!isset($this->_[IDNICH_HDOM_INFO_BEGIN])) { return array(); }

			$head = array($this->_[IDNICH_HDOM_INFO_BEGIN] => 1);
			$cmd = ' '; // Combinator
	...
	protected function seek($selector, &$ret, $parent_cmd, $lowercase = false)
	{
		...
		if ($parent_cmd === ' ') { // Descendant Combinator
			...
			$end = (!empty($this->_[IDNICH_HDOM_INFO_END])) ? $this->_[IDNICH_HDOM_INFO_END] : 0;
			if ($end == 0) {
				$parent = $this->parent;
				while (!isset($parent->_[IDNICH_HDOM_INFO_END]) && $parent !== null) {
					$end -= 1;
					$parent = $parent->parent;
				}
				$end += $parent->_[IDNICH_HDOM_INFO_END];
			}

			// Get list of target nodes
			$nodes_start = $this->_[IDNICH_HDOM_INFO_BEGIN] + 1;
		...
		foreach($nodes as $node) {
			...
			if ($pass) $ret[$node->_[IDNICH_HDOM_INFO_BEGIN]] = 1;
			unset($node);
		}
	...
	function __set($name, $value)
	{
		...
		switch ($name) {
			case 'outertext': return $this->_[IDNICH_HDOM_INFO_OUTER] = $value;
			case 'innertext':
				if (isset($this->_[IDNICH_HDOM_INFO_TEXT])) {
					return $this->_[IDNICH_HDOM_INFO_TEXT] = $value;
				}
				return $this->_[IDNICH_HDOM_INFO_INNER] = $value;
		}

		if (!isset($this->attr[$name])) {
			$this->_[IDNICH_HDOM_INFO_SPACE][] = array(' ', '', '');
			$this->_[IDNICH_HDOM_INFO_QUOTE][] = IDNICH_HDOM_QUOTE_DOUBLE;
		}
...
class idnich_simple_html_dom
{
		...
		$options = 0)
	{
		...
		if($options & IDNICH_HDOM_SMARTY_AS_TEXT) { // Strip Smarty scripts
		...
		$this->parse();
		// end
		$this->root->_[IDNICH_HDOM_INFO_END] = $this->cursor;
		$this->parse_charset();
		...
		$defaultSpanText = IDNICH_DEFAULT_SPAN_TEXT)
	{
		...
		$this->root = new idnich_simple_html_dom_node($this);
		$this->root->tag = 'root';
		$this->root->_[IDNICH_HDOM_INFO_BEGIN] = -1;
		$this->root->nodetype = IDNICH_HDOM_TYPE_ROOT;
	...
	protected function parse()
	{
		while (true) {
			...
			$node = new idnich_simple_html_dom_node($this);
			++$this->cursor;
			$node->_[IDNICH_HDOM_INFO_TEXT] = $s;
	...
	protected function read_tag()
	{
		...
		if ($this->char === '/') {
			...
			if ($parent_lower !== $tag_lower) {
				...
				if (isset($this->optional_closing_tags[$parent_lower])
					...
					$this->parent->_[IDNICH_HDOM_INFO_END] = 0;
					...
					if (strtolower($this->parent->tag) !== $tag_lower) {
						...
						$this->parent->_[IDNICH_HDOM_INFO_END] = $this->cursor;
					...
					$this->parent->_[IDNICH_HDOM_INFO_END] = 0; // No end tag
					$org_parent = $this->parent;
		...
		// start tag
		$node = new idnich_simple_html_dom_node($this);
		$node->_[IDNICH_HDOM_INFO_BEGIN] = $this->cursor;
		++$this->cursor;
		...
		if (isset($tag[0]) && $tag[0] === '!') {
			$node->_[IDNICH_HDOM_INFO_TEXT] = '<' . $tag . $this->copy_until_char('>');

			if (isset($tag[2]) && $tag[1] === '-' && $tag[2] === '-') { // Comment ("<!--")
				$node->nodetype = IDNICH_HDOM_TYPE_COMMENT;
				$node->tag = 'comment';
			} else { // Could be doctype or CDATA but we don't care
				$node->nodetype = IDNICH_HDOM_TYPE_UNKNOWN;
				$node->tag = 'unknown';
			}

			if ($this->char === '>') { $node->_[IDNICH_HDOM_INFO_TEXT] .= '>'; }
		...
		if ($pos = strpos($tag, '<') !== false) {
			$tag = '<' . substr($tag, 0, -1);
			$node->_[IDNICH_HDOM_INFO_TEXT] = $tag;
			$this->link_nodes($node, false);
		...
		if (!preg_match('/^\w[\w:-]*$/', $tag)) {
			$node->_[IDNICH_HDOM_INFO_TEXT] = '<' . $tag . $this->copy_until('<>');
		...
		$node->nodetype = IDNICH_HDOM_TYPE_ELEMENT;
		$tag_lower = strtolower($tag);
		...
		do {
			...
			if ($this->pos >= $this->size - 1 && $this->char !== '>') {
				$node->nodetype = IDNICH_HDOM_TYPE_TEXT;
				$node->_[IDNICH_HDOM_INFO_END] = 0;
				$node->_[IDNICH_HDOM_INFO_TEXT] = '<' . $tag . $space[0] . $name;
				$node->tag = 'text';
				$this->link_nodes($node, false);
				return true;
			}
			...
			if ($this->doc[$this->pos - 1] == '<') {
				...
				$node->attr = array();
				$node->_[IDNICH_HDOM_INFO_END] = 0;
				$node->_[IDNICH_HDOM_INFO_TEXT] = substr(
					$this->doc,
					$begin_tag_pos,
			...
			if ($name !== '/' && $name !== '') { // this is a attribute name
				...
				if ($this->char === '=') { // attribute with value
					...
					//no value attr: nowrap, checked selected...
					$node->_[IDNICH_HDOM_INFO_QUOTE][] = IDNICH_HDOM_QUOTE_NO;
					$node->attr[$name] = true;
					if ($this->char != '>') { $this->char = $this->doc[--$this->pos]; } // prev
				}

				$node->_[IDNICH_HDOM_INFO_SPACE][] = $space;
		...
		$this->link_nodes($node, true);
		$node->_[IDNICH_HDOM_INFO_ENDSPACE] = $space[0];

		// handle empty tags (i.e. "<div/>")
		if ($this->copy_until_char('>') === '/') {
			$node->_[IDNICH_HDOM_INFO_ENDSPACE] .= '/';
			$node->_[IDNICH_HDOM_INFO_END] = 0;
		} else {
			// reset parent
		...
		if ($node->tag === 'br') {
			$node->_[IDNICH_HDOM_INFO_INNER] = $this->default_br_text;
		}
	...
	protected function parse_attr($node, $name, &$space)
	{
		...
		switch ($this->char) {
			case '"':
				$quote_type = IDNICH_HDOM_QUOTE_DOUBLE;
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				$value = $this->copy_until_char('"');
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				break;
			case '\'':
				$quote_type = IDNICH_HDOM_QUOTE_SINGLE;
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				$value = $this->copy_until_char('\'');
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				break;
			default:
				$quote_type = IDNICH_HDOM_QUOTE_NO;
		...
		if (!$is_duplicate) {
			$node->_[IDNICH_HDOM_INFO_QUOTE][] = $quote_type;
	...
	{
		return @idnich_str_get_html("<$name>$value</$name>")->firstChild();
	}

	function createTextNode($value)
	{
		return @end(idnich_str_get_html($value)->nodes);
	}
